...
export const generateOrderInvoicePDF = async (order: IOrder): Promise<Buffer> => {
    return new Promise<Buffer>(async (resolve, reject) => {
        try {
            ...
            const logoBuffer = Buffer.from(response.data as ArrayBuffer);
            ...
            doc.fontSize(20).font('Helvetica-Bold').fillColor('#000000').text('RMSTU_ECOMMERCE', { align: 'center' });
            doc.fontSize(10).text("Level-4, 34, Awal Centre, Banani, Dhaka", { align: 'center' });
            doc.fontSize(10).text("Email: support@RMSTU_ECOMMERCE.com", { align: 'center' });
            ...
            doc.fontSize(9).fillColor('#003366').text('-RMSTU_ECOMMERCE', { align: 'center' });
